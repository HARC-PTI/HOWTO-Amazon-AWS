...
wait_for_nodes () {
  ...
  mkdir /efs/scratch/app
  aws s3 cp s3://${S3_BUCKET}/${SCRIPT_PATH}${SCRIPT_NAME} .    # copy script to /app
  mv /app/$SCRIPT_NAME /efs/scratch/app/$SCRIPT_NAME # move script to shared directory
  mpiexec -n $NUM_NODES --hostfile $HOST_FILE_PATH python3 /efs/scratch/app/$SCRIPT_NAME

  # Clean up, goodbye
  sleep 1
  rm -rf /efs/scratch/app
